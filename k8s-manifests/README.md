...
## 1. 개요
- **Kubernetes 배포 정의**: 이 디렉터리는 전체 마이크로서비스 애플리케이션을 Kubernetes Cluster에 배포하기 위한 모든 매니페스트(YAML 설정 파일)를 포함하고 있음
...
- **`*-deployment.yaml`**: 각 서비스의 Pod를 어떻게 실행할지 정의 (사용할 Container 이미지, 기본 복제본 수, 리소스 요구량/한도, Liveness/Readiness Probe 등)
