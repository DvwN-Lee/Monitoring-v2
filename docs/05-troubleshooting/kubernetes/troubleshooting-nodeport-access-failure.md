...
Cluster 외부에서 애플리케이션에 접근하기 위해 Service를 `NodePort` 타입으로 설정했지만, `<Node-IP>:<NodePort>` 주소로 접속 시 연결 시간 초과(Connection Timed Out)가 발생하며 접근이 불가능한 문제가 발생했습니다. Cluster 내부에서는 Service의 ClusterIP로 정상 접근이 가능한 상태였습니다.
...
NodePort 접근 실패는 Cluster 외부에서 Node까지의 네트워크 경로 어딘가에서 통신이 차단될 때 주로 발생합니다.
...
3.  **Node 자체의 방화벽**: 워커 Node OS(Linux)에 `iptables`, `firewalld`, `ufw` 등의 방화벽이 실행 중이고, 해당 NodePort에 대한 접근을 막고 있을 수 있습니다.

4.  **잘못된 Node IP 주소 사용**: 접근하려는 Node의 IP 주소가 외부에서 접근 불가능한 사설(private) IP인 경우. 외부에서 접근하려면 반드시 Node의 공인(public) IP 주소를 사용해야 합니다.
...
1.  Solid Cloud 콘솔에 로그인하여 Kubernetes Cluster의 워커 Node들이 속한 **보안 그룹(Security Group)**을 찾습니다.
...
#### 3단계: Node의 Public IP 주소 확인

클라우드 플랫폼의 콘솔이나 CLI를 통해 워커 Node의 **Public IP 주소**를 정확히 확인하고, 이 IP로 접근을 시도하고 있는지 다시 한번 확인합니다.

#### 4단계: Node OS의 방화벽 확인 (온프레미스 등)

클라우드 환경이 아니거나, OS 수준의 방화벽이 의심될 경우 Node에 직접 접속하여 방화벽 상태를 확인합니다.
...
1.  **NodePort의 통신 흐름을 이해하라**: NodePort 접근은 `외부 -> 클라우드 방화벽 -> Node -> kube-proxy(iptables/IPVS) -> Pod` 순서로 이루어집니다. 문제가 발생하면 이 흐름의 각 단계를 순서대로 점검하는 것이 효율적입니다.
...
3.  **NodePort는 테스트/개발용**: NodePort는 모든 Node에 포트를 열어야 하고 포트 관리도 번거로워 프로덕션 환경에는 잘 사용되지 않습니다. 프로덕션 환경에서는 `LoadBalancer` 타입의 Service나 `Ingress`를 사용하여 외부 트래픽을 처리하는 것이 일반적이고 더 안전합니다.
