...
새로운 Pod를 배포하거나 기존 Deployment의 `replicas` 수를 늘리는 과정에서 Pod가 생성되지 않고 `Pending` 상태에 머무르는 문제가 발생했습니다. `kubectl describe pod`를 통해 확인한 결과, Namespace에 설정된 `ResourceQuota`를 초과하여 Pod 생성이 거부되고 있었습니다.
...
1.  **Namespace 리소스 할당량 초과**: 관리자가 Namespace별로 리소스 사용을 제한하기 위해 설정한 `ResourceQuota`의 한도에 도달한 것이 가장 직접적인 원인입니다. Quota는 CPU/메모리의 `requests`/`limits` 총합, 스토리지(PVC) 개수, 서비스나 시크릿 같은 오브젝트 수 등 다양한 항목에 대해 설정될 수 있습니다.

2.  **Istio Sidecar의 리소스 요구량**: Istio와 같은 서비스 메쉬를 사용하는 경우, 모든 Pod에 `istio-proxy`라는 사이드카 Container가 자동으로 주입됩니다. 이 사이드카는 자체적으로 CPU와 메모리 리소스를 요구(`requests`)하고 한도(`limits`)를 가집니다. 애플리케이션 Container의 리소스만 고려하고 사이드카의 리소스를 간과하면 예상치 못하게 Quota를 빠르게 소진할 수 있습니다. 특히 Istio의 기본 CPU `limits`는 비교적 높게 설정되어 있을 수 있습니다.

3.  **리소스 요청/한도(requests/limits) 미설정**: `ResourceQuota`가 적용된 Namespace에서는 모든 Container에 대해 `requests`와 `limits`를 명시적으로 설정해야 하는 경우가 많습니다. 만약 이를 누락하면 Pod 생성 자체가 거부될 수 있습니다.

## 4. 해결 방법

#### 1단계: Namespace의 ResourceQuota 확인

먼저 해당 Namespace에 어떤 `ResourceQuota`가 적용되어 있고, 현재 사용량은 얼마인지 확인합니다.

```bash
# Namespace의 모든 ResourceQuota 오브젝트 이름 확인
...
Quota를 무작정 늘리기 전에, 현재 Namespace 내에서 리소스를 비효율적으로 사용하고 있는 Pod가 있는지 검토합니다.
...
리소스 최적화 후에도 추가 리소스가 반드시 필요하다면, Cluster 관리자에게 `ResourceQuota`의 `Hard` 한도를 늘려달라고 요청해야 합니다. 이는 일반적으로 인프라 용량과 비용에 영향을 미치므로 신중하게 결정해야 합니다.
...
1.  **Quota는 공유 자원의 보호 장치**: `ResourceQuota`는 특정 팀이나 애플리케이션이 Cluster 전체 리소스를 독점하는 것을 방지하는 중요한 기능입니다. Quota 초과 오류는 '실패'가 아니라, 할당된 자원을 모두 소진했다는 '알림'으로 이해해야 합니다.
2.  **눈에 보이지 않는 비용, 사이드카**: 서비스 메쉬, 모니터링 에이전트 등 사이드카 패턴으로 주입되는 Container들은 애플리케이션의 전체 리소스 요구량을 크게 증가시킬 수 있습니다. Pod 배포 시 항상 모든 Container의 리소스 요구량을 종합적으로 고려해야 합니다.
