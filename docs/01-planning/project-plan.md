...
|----------|------|----------|-----------------|
| **1.0** | 2025-10-01 | 초안 작성 (AWS 기반 계획) | AWS EKS 환경을 기준으로 개발 계획 수립 |
...
### 1.2. 타임라인
- **시작일**: 2025년 9월 30일
- **종료 예정일**: 2025년 12월 19일

### 1.3. 개발 환경
- **개발/테스트**: CloudStack 기반 Solid Cloud
...
#### 목표
Terraform을 사용하여 Solid Cloud에 Kubernetes Cluster와 기본 인프라 구축
...
#### 완료 기준
- [ ] `terraform apply`로 Cluster 생성 성공
- [ ] kubectl로 Cluster 접근 가능
- [ ] PostgreSQL이 정상적으로 실행되고 데이터 저장 확인

---

### Week 2: CI/CD Pipeline 구축 (10/6 ~ 10/12)

#### 목표
GitHub Actions와 Argo CD를 연동하여 자동 배포 Pipeline 완성
...
3. **보안 검증**
   - Trivy 스캔 결과 확인 (Critical/High 등급 취약점 0개)
   - mTLS 암호화 확인
...
### Must-Have (필수)
- [ ] Terraform으로 Kubernetes Cluster 생성
