...
Microservice 아키텍처에서 데이터베이스 부하를 줄이고 응답 속도를 개선하기 위해 인메모리 캐시 솔루션이 필요했습니다. 특히 자주 조회되는 데이터(블로그 포스트 목록, 카테고리 정보 등)를 캐싱하고, 세션 데이터를 공유하기 위한 중앙 집중식 저장소가 요구되었습니다.
...
- Pub/Sub 기능으로 캐시 무효화 메시지를 여러 Service에 브로드캐스트 가능
