...
Microservice 아키텍처에서 Service 간 통신을 안전하고 효율적으로 관리하기 위해 Service Mesh 도입이 필요했습니다. 트래픽 관리, 보안(mTLS), 관측성(Observability), 그리고 장애 복구 기능을 통합적으로 제공하는 솔루션을 찾고 있었습니다.
...
| **관측성** | **Kiali, Jaeger, Grafana 통합** | Linkerd Dashboard 제공 | Kiali를 통한 Service Mesh 시각화가 데모와 운영에 효과적임. |
...
**Kiali를 통한 Service Mesh 시각화**와 **세밀한 트래픽 제어 기능**이 결정적이었습니다. Kiali 대시보드를 통해 Service 간 트래픽 흐름, mTLS 상태, 에러율을 실시간으로 시각화할 수 있어 운영과 디버깅에 매우 유용합니다. 또한 VirtualService와 DestinationRule을 활용한 카나리 배포, A/B 테스트 등 고급 트래픽 관리 기능을 실습할 수 있다는 점도 학습 목적에 부합했습니다.
...
### 긍정적 측면
- Kiali를 통해 Service Mesh의 트래픽 패턴과 mTLS 상태를 시각적으로 모니터링 가능
- VirtualService를 통한 정교한 라우팅 규칙 설정 (헤더 기반, 가중치 기반)
- Prometheus와의 완벽한 통합으로 풍부한 메트릭 수집 (요청 수, 지연시간, 에러율)
- Namespace 단위 mTLS STRICT 모드 적용으로 Service 간 통신 보안 강화
