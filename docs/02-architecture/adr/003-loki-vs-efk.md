...
Microservice 환경에서는 로그가 여러 Pod에 분산되어 발생하므로, 장애 추적 및 디버깅을 위해 모든 로그를 한 곳에서 효율적으로 수집하고 검색할 수 있는 중앙 로깅 시스템이 필수적입니다. Prometheus(메트릭)와 Jaeger(추적)와 함께 관측성(Observability)을 만족해야 합니다.
...
-   가벼운 아키텍처 덕분에 제한된 Cluster 리소스 내에서 효율적으로 로깅 시스템을 운영할 수 있음.
